...
func (g *Graphs) TnrDijkstra(source int64, target int64) *types.Path {
	...
	g.tnrAppendPathVanillaDijkstra(&path, g.Tnr, source, accessSourceNode)
	...
	g.tnrAppendPathVanillaDijkstra(&path, g.Tnr, accessTargetNode, target)
...
func (g *Graphs) tnrAppendPathVanillaDijkstra(path *types.Path, graph *ch.Graph, source int64, target int64) {
	cost, nodes := graph.VanillaShortestPath(source, target)
	n := len(nodes)
